...
export default function ArchitectFormModal({ isOpen, onClose, onSubmit, initialData }) {
            ...
            <div className="bg-[var(--dashboard-surface)] w-full max-w-lg rounded-2xl shadow-2xl p-6 border border-[var(--dashboard-border)] overflow-y-auto max-h-[90vh]">
                ...
                <div className="flex items-center justify-between mb-6">
                    <h2 className="text-lg font-black tracking-tight text-[var(--dashboard-text)]">
                        {initialData ? "Edit Persona Lokal" : "Daftarkan Persona Arsitek Lokal"}
                    </h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-4 py-3 bg-[var(--dashboard-surface-soft)] border border-[var(--dashboard-border-soft)] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20"
                            required
                            placeholder="Contoh: Ar. Ridwan Kamil"
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            onChange={handleChange}
                            className="w-full px-4 py-3 bg-[var(--dashboard-surface-soft)] border border-[var(--dashboard-border-soft)] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20"
                            placeholder="Contoh: Modern Tropical, Industrial, dll"
                        />
                    </div>

                    <div>
                        <label className="block text-[10px] font-bold text-[var(--dashboard-text-soft)] uppercase tracking-widest mb-1">Bio Singkat</label>
                        <textarea
                            name="bio"
                            value={form.bio}
                            onChange={handleChange}
                            rows="2"
                            className="w-full px-4 py-3 bg-[var(--dashboard-surface-soft)] border border-[var(--dashboard-border-soft)] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20 resize-none"
                            placeholder="Pengalaman atau gaya desain..."
                        />
                    </div>
