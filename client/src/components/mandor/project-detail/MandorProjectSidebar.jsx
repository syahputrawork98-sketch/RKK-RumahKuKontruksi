...
import { FiUsers, FiCreditCard, FiClock, FiCheckCircle } from "react-icons/fi";

const MandorProjectSidebar = ({ project, stages = [], journals = [] }) => {
    const finishedStages = stages.filter(s => s.status?.toLowerCase().includes('finish') || s.status?.toLowerCase().includes('selesai') || s.status?.toLowerCase().includes('completed')).length;
    const submittedJournals = journals.length;
            ...
            <div className="dashboard-card shadow-sm border-[var(--dashboard-border)]">
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl border border-slate-100/50">
                        <div className="w-10 h-10 rounded-xl bg-purple-50 text-purple-600 flex items-center justify-center shrink-0 shadow-sm">
                            <FiUsers />
                        </div>
                        <div>
                            <p className="text-[9px] font-black text-slate-400 uppercase">Pengawas Proyek</p>
                            <h4 className="text-xs font-black text-slate-700">{project.supervisor?.name || "BELUM ADA"}</h4>
                        </div>
                    </div>
                    <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl border border-slate-100/50">
                        ...
                        <div>
                            ...
                            <h4 className="text-xs font-black text-slate-700">{project.admin?.name || "PUSAT"}</h4>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="relative z-10">
                    ...
                    <div className="space-y-5 mt-6">
                        <div className="flex justify-between items-center">
                            <span className="text-[10px] font-bold opacity-60 uppercase tracking-tighter">Jurnal Terkirim</span>
                            <span className="text-2xl font-black">{submittedJournals} <span className="text-[10px] opacity-70">Laporan</span></span>
                        </div>
                        <div className="flex justify-between items-center">
                            <span className="text-[10px] font-bold opacity-60 uppercase tracking-tighter">Kategori Selesai</span>
                            <span className="text-2xl font-black">{finishedStages} <span className="text-[10px] opacity-70">/ {stages.length}</span></span>
                        </div>
                    </div>
                </div>
