                ...
                {activeTab === "PEMBAYARAN_KONSUMEN" && (
                    <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/40 overflow-hidden">
                        <div className="p-8 border-b border-slate-50 flex justify-between items-center">
                            ...
                            <button className="px-5 py-2 bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all">Filter Status</button>
                        </div>
                            ...
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="bg-slate-50/50 border-b border-slate-50">
                                        ...
                                        <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Bukti & Aksi</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-50">
                                    {payments.filter(p => p.type === 'CUSTOMER_PAYMENT').length === 0 ? (
                                        <tr>
                                            <td colSpan="5" className="px-8 py-20 text-center text-slate-400 text-xs font-bold uppercase tracking-widest">Belum ada pembayaran masuk</td>
                                        </tr>
                                    ) : (
                                        payments.filter(p => p.type === 'CUSTOMER_PAYMENT').map(p => (
                                            <tr key={p.id} className="hover:bg-slate-50/50 transition-all cursor-pointer" onClick={() => { setSelectedPayment(p); setShowDetail(true); }}>
                                                ...
                                                <td className="px-8 py-6">
                                                    ...
                                                    <p className="text-[10px] font-bold text-slate-400 uppercase">{p.customer?.name}</p>
                                                </td>
